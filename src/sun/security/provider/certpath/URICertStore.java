...
class URICertStore extends CertStoreSpi {
    ...
    // cached X509CRL_YA (may be null)
    private X509CRL_YA crl;
    ...
    /**
     * Returns a <code>Collection</code> of <code>X509CRL_YA</code>s that
     * match the specified selector. If no <code>X509CRL_YA</code>s
     * match the selector, an empty <code>Collection</code> will be returned.
     *
     * @param selector A <code>CRLSelector</code> used to select which
     *  <code>X509CRL_YA</code>s should be returned. Specify <code>null</code>
     *  to return all <code>X509CRL_YA</code>s.
     * @return A <code>Collection</code> of <code>X509CRL_YA</code>s that
     *         match the specified selector
     * @throws CertStoreException if an exception occurs
     */
    public synchronized Collection<X509CRL_YA> engineGetCRLs(CRLSelector selector)
        ...
        if (ldap) {
            ...
            // caching mechanism, see the class description for more info.
            return (Collection<X509CRL_YA>) ldapCertStore.getCRLs(xsel);
        }
        ...
        try {
            ...
            crl = (X509CRL_YA) factory.generateCRL(in);
            return getMatchingCRLs(crl, selector);
        } catch (IOException e) {
            if (debug != null) {
        ...
        lastModified = 0;
        crl = null;
        return Collections.<X509CRL_YA>emptyList();
    }

    /**
     * Checks if the specified X509CRL_YA matches the criteria specified in the
     * CRLSelector.
     */
    private static Collection<X509CRL_YA> getMatchingCRLs
        (X509CRL_YA crl, CRLSelector selector) {
        if (selector == null || (crl != null && selector.match(crl))) {
            return Collections.<X509CRL_YA>singletonList(crl);
        } else {
            return Collections.<X509CRL_YA>emptyList();
        }
    }
