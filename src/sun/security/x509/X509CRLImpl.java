...
import java.security.cert.X509CRL_YA;
import java.security.cert.X509Certificate;
import java.security.cert.X509CRLEntry_YA;
 ...
 * @see X509CRL_YA
 */
public class X509CRLImpl extends X509CRL_YA implements DerEncoder {
    ...
    private Map<X509IssuerSerial,X509CRLEntry_YA> revokedCerts = new LinkedHashMap<X509IssuerSerial,X509CRLEntry_YA>();
     ...
     */
    public X509CRLImpl(X500Name issuer, Date thisDate, Date nextDate,
               X509CRLEntry_YA[] badCerts, CRLExtensions crlExts)
        throws CRLException
    {
    ...
    public void encodeInfo(OutputStream out) throws CRLException {
        try {
            ...
            if (!revokedCerts.isEmpty()) {
                for (X509CRLEntry_YA entry : revokedCerts.values()) {
    ...
    public String toString() {
        ...
        if (revokedCerts.isEmpty())
            ...
            int i = 1;
            for (Iterator<X509CRLEntry_YA> iter = revokedCerts.values().iterator();
                                             iter.hasNext(); i++)
    ...
    /**
     * Return the issuer as X500Principal. Overrides method in X509CRL_YA
     ...
     * @see X509CRLEntry_YA
     */
    public X509CRLEntry_YA getRevokedCertificate(BigInteger serialNumber) {
        if (revokedCerts.isEmpty()) {
            return null;
        }
     ...
     */
    public X509CRLEntry_YA getRevokedCertificate(X509Certificate cert) {
        if (revokedCerts.isEmpty()) {
            return null;
        }
     ...
     * A Set of X509CRLEntry_YA.
     *
     * @return all the revoked certificates or <code>null</code> if there are
     * none.
     * @see X509CRLEntry_YA
     */
    public Set<X509CRLEntry_YA> getRevokedCertificates() {
        if (revokedCerts.isEmpty()) {
            return null;
        } else {
            return new HashSet<X509CRLEntry_YA>(revokedCerts.values());
        }
    }
    ...
    /**
     * Extract the issuer X500Principal from an X509CRL_YA. Parses the encoded
     * form of the CRL to preserve the principal's ASN.1 encoding.
     *
     * Called by java.security.cert.X509CRL_YA.getIssuerX500Principal().
     */
    public static X500Principal getIssuerX500Principal(X509CRL_YA crl) {
        try {
     ...
     */
    public static byte[] getEncodedInternal(X509CRL_YA crl) throws CRLException {
    ...
    /**
     * Utility method to convert an arbitrary instance of X509CRL_YA
     * to a X509CRLImpl. Does a cast if possible, otherwise reparses
     * the encoding.
     */
    public static X509CRLImpl toImpl(X509CRL_YA crl)
            throws CRLException {
        if (crl instanceof X509CRLImpl) {
