...
class DistributionPointFetcher {
     ...
     * empty set.
     */
    Collection<X509CRL_YA> getCRLs(X509CRLSelector selector, boolean signFlag,
        ...
        try {
            ...
            Set<X509CRL_YA> results = new HashSet<X509CRL_YA>();
            for (Iterator<DistributionPoint> t = points.iterator();
                 t.hasNext() && !Arrays.equals(reasonsMask, ALL_REASONS); ) {
                DistributionPoint point = t.next();
                Collection<X509CRL_YA> crls = getCRLs(selector, certImpl,
     ...
     */
    private Collection<X509CRL_YA> getCRLs(X509CRLSelector selector,
        ...
        Collection<X509CRL_YA> possibleCRLs = new ArrayList<X509CRL_YA>();
        Collection<X509CRL_YA> crls = new ArrayList<X509CRL_YA>(2);
        ...
        for (X509CRL_YA crl : possibleCRLs) {
            try {
     ...
     */
    private X509CRL_YA getCRL(URIName name) {
        URI uri = name.getURI();
        if (debug != null) {
        ...
        try {
            ...
            if (crls.isEmpty()) {
                ...
                return (X509CRL_YA) crls.iterator().next();
            }
        } catch (Exception e) {
            if (debug != null) {
     ...
     */
    private Collection<X509CRL_YA> getCRLs(X500Name name,
        X500Principal certIssuer, List<CertStore> certStores)
    {
        ...
        Collection<X509CRL_YA> crls = new ArrayList<X509CRL_YA>();
        for (CertStore store : certStores) {
            try {
                for (CRL crl : store.getCRLs(xcs)) {
                    crls.add((X509CRL_YA)crl);
                }
    ...
    boolean verifyCRL(X509CertImpl certImpl, DistributionPoint point,
        X509CRL_YA crl, boolean[] reasonsMask, boolean signFlag,
