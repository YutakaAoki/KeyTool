...
public class IndexedCollectionCertStore extends CertStoreSpi {
    ...
    /**
     * Map X500Principal(issuer) -> X509CRL_YA | List of X509CRL_YA
     */
    ...
    private void buildIndex(Collection<?> coll) {
        ...
        for (Object obj : coll) {
            ...
            } else if (obj instanceof X509CRL_YA) {
                indexCRL((X509CRL_YA)obj);
    ...
    /**
     * Add an X509CRL_YA to the index.
     */
    private void indexCRL(X509CRL_YA crl) {
        X500Principal issuer = crl.getIssuerX500Principal();
        Object oldEntry = crlIssuers.put(issuer, crl);
        if (oldEntry != null) { // assume this is unlikely
            if (oldEntry instanceof X509CRL_YA) {
                if (crl.equals(oldEntry)) {
                    return;
                }
                List<X509CRL_YA> list = new ArrayList<X509CRL_YA>(2);
                list.add(crl);
                list.add((X509CRL_YA)oldEntry);
                crlIssuers.put(issuer, list);
            } else {
                List<X509CRL_YA> list = (List<X509CRL_YA>)oldEntry;
    ...
    public Collection<CRL> engineGetCRLs(CRLSelector selector)
        ...
        if (issuers != null) {
            ...
            for (X500Principal issuer : issuers) {
                ...
                if (entry == null) {
                    // empty
                } else if (entry instanceof X509CRL_YA) {
                    X509CRL_YA crl = (X509CRL_YA)entry;
                    if (x509Selector.match(crl)) {
                        matches.add(crl);
                    }
                } else { // List
                    List<X509CRL_YA> list = (List<X509CRL_YA>)entry;
                    for (X509CRL_YA crl : list) {
                        if (x509Selector.match(crl)) {
                            matches.add(crl);
                        }
                    }
    ...
    private void matchX509CRLs(CRLSelector selector, Collection<CRL> matches) {
        for (Object obj : crlIssuers.values()) {
            if (obj instanceof X509CRL_YA) {
                X509CRL_YA crl = (X509CRL_YA)obj;
                if (selector.match(crl)) {
                    matches.add(crl);
                }
            } else {
                List<X509CRL_YA> list = (List<X509CRL_YA>)obj;
                for (X509CRL_YA crl : list) {
                    if (selector.match(crl)) {
                        matches.add(crl);
                    }
